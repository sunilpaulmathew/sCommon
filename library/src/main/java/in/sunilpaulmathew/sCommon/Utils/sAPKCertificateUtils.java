 ...
 */
public class sAPKCertificateUtils {
    ...
    public sAPKCertificateUtils(File apkFile, String packageName, Context context) {
        mAPKFile = apkFile;
        mPackageName = packageName;
        mContext = context;
    }

    private String getCertificateFingerprint(X509Certificate cert, String hashAlgorithm) {
        String hash;
        try {
    ...
    private X509Certificate[] getX509Certificates() {
        ...
        try {
            ...
            if (mPackageName != null && sPackageUtils.isPackageInstalled(mPackageName, mContext)) {
                packageInfo = mContext.getPackageManager()
                        .getPackageInfo(mPackageName, PackageManager.GET_SIGNATURES);
            } else if (mAPKFile != null && mAPKFile.exists()) {
                packageInfo = mContext.getPackageManager().getPackageArchiveInfo(mAPKFile.getAbsolutePath(), PackageManager.GET_SIGNATURES);
            }
            if (packageInfo != null) {
